...
db_folder = os.environ.get("DB_FOLDER", INSTANCE_DIR)
logs_folder = os.environ.get("LOGS_FOLDER", os.path.join(BASE_DIR, 'data/persist/logs'))
...
app.config["SQLALCHEMY_DATABASE_URI"] = f"sqlite:///{os.path.join(db_folder, 'cardwarskingdom.db')}"
...
def AdminPage():
    ...
    if not Admin.query.first():
        Log('admin', 'first admin login')
        randompassword = ''.join(random.choices(string.ascii_letters + string.digits, k=24))
        Log('admin', 'randompassword: ' + randompassword)
        try:
            newAdmin = Admin(username="admin", password=bcrypt.generate_password_hash(randompassword).decode('utf-8'),
                             rank=0)
            ...
            Log('admin','Error creating admin account: ', e)
...
def AdminVersions():
    ...
    if request.method == 'GET':
        ...
        return render_template('admin_versions.html', pc_version=open("data/persist/version.txt", "r").read(),
                               android_version=open("data/persist/android_version.txt", "r").read())
...
def AdminCreateAdmin():
    ...
    if request.method == 'POST':
        ...
        new_admin = Admin(username=username, password=bcrypt.generate_password_hash(password).decode('utf-8'),
                          rank=int(rank))
...
def AdminPlayers():
    ...
    for player in players:
        player["last_online"] = datetime.fromtimestamp(player["last_online"], tz=timezone.utc).strftime(
            '%Y-%m-%d %H:%M:%S UTC')
...
def AdminPlayer(player):
    ...
    for battle in battle_history:
        battle["recordTime"] = datetime.fromtimestamp(battle["recordTime"], tz=timezone.utc).strftime(
            '%Y-%m-%d %H:%M:%S UTC')
    ...
    return render_template('admin_player.html', player=player, is_banned=IsUserBanned(player["username"]),
                           SoftCurrency=game["SoftCurrency"],
                           HardCurrency=int(game["PaidHardCurrency"]) + int(game["FreeHardCurrency"]),
                           PvpCurrency=game["PvpCurrency"],
                           InstalledDate=datetime.fromtimestamp(game["InstalledDate"], tz=timezone.utc).strftime(
                               '%Y-%m-%d %H:%M:%S UTC'), PVPBanned=bool(game["Zxcvbnm"]),
                           MultiplayerLevel=game["MultiplayerLevel"], InventorySpace=game["InventorySpace"],
                           BattleHistory=battle_history, DeviceName=player["devicename"], Inventory=Inventory)
...
def Backup():
    ...
    os.makedirs(f"{backup_folder}/" + now, exist_ok=True)

    # copy database
    shutil.copy(f"{db_folder}/cardwarskingdom.db", f"{backup_folder}/{now}/cardwarskingdom.db")

    # copy persist folder
    shutil.copytree(data_folder, f"{backup_folder}/{now}/persist")

    # zip
    shutil.make_archive(f"{backup_folder}/" + now, 'zip', f"{backup_folder}/" + now)

    # delete folder
    shutil.rmtree(f"{backup_folder}/" + now)
...
def AdminLogs():
    ...
    if query != '':
        logs = db.paginate(db.select(Logs).filter(Logs.player == query).order_by(Logs.id.desc()), page=pagerequest,
                           per_page=perpage)
...
def AdminUpsight():
    ...
    if query != '':
        logs = db.paginate(
            db.select(UpsightLogs).filter(UpsightLogs.player_id == query).order_by(UpsightLogs.id.desc()),
            page=pagerequest, per_page=perpage)
...
def PlayerLog(ip: str, player: str, message: str):
    db_log = Logs(date=datetime.now().strftime("%Y-%m-%d"), time=datetime.now().strftime("%H:%M:%S"), player=player,
...
def PersistMessagesReceivedIDs():
    return send_from_directory(directory="", path="data/persist/messages_received_ids.json", as_attachment=True,
                               download_name="messages_received_ids.json")
...
def PersistMessagesGet(message):
    ...
    return send_from_directory(directory="", path="data/persist/messages/{message}.json", as_attachment=True,
                               download_name=f"{message}.json")
...
def MultiplayerUpdateDeckName():
    ...
    if db_user.leader_level is None and int(clientData["leader_level"]) > 5:
        DiscordWebhookMessage(
            f"{clientData['player_id']} attempted to set leader level to a {clientData['leader_level']} on a fresh account! IP: " + IPFromRequest(
                request))
        SystemBan(clientData["player_id"])
    elif db_user.leader_level is not None and int(clientData["leader_level"]) > int(
            db_user.leader_level) + 10:  # Make sure leader level is incremented by more than 10, if not, ban user
        DiscordWebhookMessage(
            f"{clientData['player_id']} attempted to set leader level to {clientData['leader_level']} when it was set to {db_user.leader_level}! IP: " + IPFromRequest(
                request))
        SystemBan(clientData["player_id"])
    ...
    PlayerLog(IPFromRequest(request), clientData["player_id"],
              f"Set Deck Rank to {clientData['deck_rank']}\nSet Landscapes to {clientData['landscapes']}\nSet Helper Creature to {clientData['helper_creature']}\nSet Leader to {clientData['leader']}\nSet Leader Level to {clientData['leader_level']}\nSet Allyboxspace to {clientData['allyboxspace']}")
...
def PersistGame():
    ...
    if request.headers.get("User-Agent") != "Innertube Explorer v0.1":
        DiscordWebhookMessage(
            username + " attempted to access game without User-Agent header. IP: " + IPFromRequest(request))
        abort(404)
    if request.headers.get("Platform") is None:
        DiscordWebhookMessage(
            username + " attempted to access game without Platform header. IP: " + IPFromRequest(request))
        abort(404)
    if request.headers.get("Version") is None:
        DiscordWebhookMessage(
            username + " attempted to access game without Version header. IP: " + IPFromRequest(request))
        abort(404)
    if request.method == 'PUT':
        if request.headers.get("X-Nick-Description") is None:
            DiscordWebhookMessage(
                username + " attempted to access game without X-Nick-Description header. IP: " + IPFromRequest(request))
            abort(404)
    ...
    if request.method == 'PUT':
        ...
        if DeviceNameUser.devicename != devicename:
            DiscordWebhookMessage(
                username + " attempted to access game with wrong device name. Device name: '" + devicename + "'. IP: " + IPFromRequest(
                    request))
            return make_response("Invalid Username!", 400)
        ...
        try:
            ...
            if game is not None:
                if int(game["Zxcvbnm"]) == 1:
                    DiscordWebhookMessage(
                        username + " attempted to access game while PVP banned. IP: " + IPFromRequest(request))
                    SystemBan(username)
                    return make_response("User is banned!", 400)
        except Exception as e:
    ...
    if request.method == 'PUT':
        ...
        if not data.startswith(b"username=") or data.startswith(b"{"):
            DiscordWebhookMessage(username + " attempted to put game data without encryption. IP: " + IPFromRequest(
                request) + ". Data: " + data.decode("utf-8")[:50])
...
def PersistFriendsFindCandidates():
    ...
    strangers = Player.query.filter(
        ...
        Player.leader_level.between(db_user.leader_level - int(clientData["level"]),
                                    db_user.leader_level + int(clientData["level"]))
        # level is within clientData["level"]
...
def AnalyticsPVPMatch():
    ...
    # write to file
    os.makedirs(f"{gamedata_folder}/pvpmatches", exist_ok=True)

    with open(f"{gamedata_folder}/pvpmatches/" + headers.get("Player-Id", "unknown") + "_" + headers.get("Match-Id",
                                                                                                   "unknown") + ".json",
              "w") as outfile:
...
def Log(category, message):
    os.makedirs(f"{logs_folder}", exist_ok=True)
    date = datetime.now().strftime("%Y-%m-%d")
    time = datetime.now().strftime("%H:%M:%S")
    with open(f"{logs_folder}/" + date + ".txt", "a") as f:
